...
def init_plasma(p_max, dp, x_min, x_max, dx, Temp):
  ...
  def vector_potential(x, t):
    if x < 0:
      #         Cos[t - x] - 12/13 Cos[t + x] - 5/13 Sin[t + x]
      return 6*(cos(t-x) - 12./13.*cos(t+x) - 5./13.*sin(t+x))
    else:
      #         1/13 E^(-5 x) Cos[t] - 5/13 E^(-5 x) Sin[t]
      return 6*(1./13*exp(-5*x)*cos(t) - 5./13*exp(-5*x)*sin(t))
