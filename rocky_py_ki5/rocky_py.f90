...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  IF (x <= 0) THEN
                ! 5/13 Cos[t + x] + Sin[t - x] - 12/13 Sin[t + x]
   E(2) =  E0 * ( 5._d/13._d*cos(t+x) + sin(t-x) - 12._d/13._d*sin(t+x) )
  ELSE
                ! 5/13 E^(-5 x) Cos[t] + 1/13 E^(-5 x) Sin[t]
   E(2) =  E0 * ( 5._d/13._d*exp(-5._d*x)*cos(t) + 1._d/13._d*exp(-5._d*x)*sin(t) )
  END IF
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  IF (x <= 0) THEN
                ! -(5/13) Cos[t + x] + Sin[t - x] + 12/13 Sin[t + x]
   B(3) =  B0 * ( -(5._d/13._d)*cos(t+x) + sin(t-x) + 12._d/13._d*sin(t+x) )
  ELSE
                ! -(5/13) E^(-5 x) Cos[t] + 25/13 E^(-5 x) Sin[t]
   B(3) =  B0 * ( -(5._d/13._d)*exp(-5._d*x)*cos(t) + 25._d/13._d*exp(-5._d*x)*sin(t) )
  END IF
