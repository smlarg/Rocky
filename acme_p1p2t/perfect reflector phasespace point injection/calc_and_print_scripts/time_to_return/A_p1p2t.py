...
def find_time_tuple_wrapper(tuppy):
  return find_reentry_time(tuppy[0], tuppy[1])
...
def par_slice(t, p1_max, p2_max, dp):
  ...
  for i in range(p1_n):
    ...
    the_result = pool.map(find_time_tuple_wrapper, zip(p, [t for dummy in range(p2_n)]))
    for j in range(p2_n):
      if (not the_result[j][1]):
        slice[i,j] = the_result[j][0]
      else:
        slice[i,j] = the_result[j][0]
        mask[i,j] = 1.0
  pool.close()
