...
if __name__ == "__main__":
  ...
  p1_max, p2_max, dp = 4.5, 4.5, 0.05
  import os
  pathToFrames = os.getcwd()
  no_o_frames = 100
  for i in range(no_o_frames):
    t = 2*math.pi*i/no_o_frames
    slice0 = par_slice(t, p1_max, p2_max, dp)
    #fig, ax = plt.subplots()
    ...
    plt.imshow(slice0, cmap=plt.cm.RdBu, vmin=0., vmax=12., extent=[-p2_max, p2_max, -p1_max,0.])
    plt.colorbar()
    index = "%03d" % i
