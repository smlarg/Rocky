...
def get_2d_ps(part_data, axes_params):
  '''
  ...
  for i in xrange(data0.shape[0]):
    index0 = int((data0[i] - ax0.min)/ax0.dx())
    index1 = int((data1[i] - ax1.min)/ax1.dx())
      ...
      if j in xrange( ax0.n ):
        ...
        for k in (index1, index1 + 1):
          if k in xrange(ax1.n):
            ps[j,k] += part_data['q'][i]*frac0*frac1
          frac1 = 1. - frac1
      frac0 = 1. - frac0
  return ps
...
if __name__ == '__main__':
  ...
  ps_ds = get_2d_ps( part_dict, axes_params )
