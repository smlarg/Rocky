...
def init_plasma(p_max, dp, x_min, x_max, dx, Temp):
  ...
  def vector_potential(x, t):
    if x < 0:
      #         Cos[t - x] - 99/101 Cos[t + x] - 20/101 Sin[t + x]
      return 6*(cos(t-x) - 99./101*cos(t+x) - 20./101*sin(t+x))
    else:
      #         2/101 E^(-10 x) Cos[t] - 20/101 E^(-10 x) Sin[t]
      return 6*(2./101*exp(-10.*x)*cos(t) - 20./101*exp(-10.*x)*sin(t))
  ...
  for i in range(nx):
    for j in range(np):
      for k in range(np):
        ...
        p_i[1] -= vector_potential(x_i[0],0)
        if q_i > 0.:
          x[index] = x_i
          p[index] = p_i
          q[index] = q_i
