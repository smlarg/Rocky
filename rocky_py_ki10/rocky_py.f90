...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  IF (x <= 0) THEN
   E(2) =  E0 * ( 20._d/101._d*cos(t+x) + sin(t-x) - 99._d/101._d*Sin(t+x))
  ELSE
   E(2) =  E0 * ( 20._d/101._d * exp(-10._d*x)*cos(t) + 2._d/101._d*exp(-10._d*x)*sin(t) )
  END IF
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  IF (x <= 0) THEN
   B(3) =  B0 * ( -(20._d/101._d)*cos(t+x) + sin(t-x) + 99._d/101._d*sin(t+x) )
  ELSE
   B(3) =  B0 * ( -(20._d/101._d)*exp(-10._d*x)*cos(t) + 200._d/101._d*exp(-10._d*x)*sin(t) )
  END IF
