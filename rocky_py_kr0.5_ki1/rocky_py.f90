...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  IF (x <= 0) THEN
                ! 8/13 Cos[t + x] + Sin[t - x] - 1/13 Sin[t + x]
   E(2) =  E0 * ( 8._d/13._d*cos(t+x) + sin(t-x) - 1._d/13._d*sin(t+x))
  ELSE
                ! 8/13 E^-x Cos[t - x/2] + 12/13 E^-x Sin[t - x/2]
   E(2) =  E0 * ( 8._d/13._d*exp(-x)*cos(t-x/2._d) + 12._d/13._d*exp(-x)*sin(t-x/2._d) )
  END IF
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  IF (x <= 0) THEN
                ! -(8/13) Cos[t + x] + Sin[t - x] + 1/13 Sin[t + x]
   B(3) =  B0 * ( -(8._d/13._d)*cos(t+x) + sin(t-x) + 1._d/13._d*sin(t+x) )
  ELSE
                ! -(8/13) E^-x Cos[t - x/2] + 14/13 E^-x Sin[t - x/2]
   B(3) =  B0 * ( -(8._d/13._d)*exp(-x)*cos(t-x/2._d) + 14._d/13._d*exp(-x)*sin(t-x/2._d) )
  END IF
