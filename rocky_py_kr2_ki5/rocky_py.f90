...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  IF (x <= 0) THEN 
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 4.94117647058824*sin(1.0*t + 1.0*x) + 1.76470588235294*cos(1.0*t + 1.0*x)
  ELSE
    E(2) = (1.05882352941176*sin(1.0*t - 2.0*x) + 1.76470588235294*cos(1.0*t - 2.0*x))*exp(-5.0*x)
  END IF
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  IF (x <= 0) THEN
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 4.94117647058824*sin(1.0*t + 1.0*x) - 1.76470588235294*cos(1.0*t + 1.0*x)
  ELSE
    B(3) = (10.9411764705882*sin(1.0*t - 2.0*x) - 1.76470588235294*cos(1.0*t - 2.0*x))*exp(-5.0*x)
  END IF
