...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 1.75735931288071*sin(1.0*t + 1.0*x) + 1.75735931288071*cos(1.0*t + 1.0*x)
  else
    E(2) = (2.48528137423857*sin(1.0*t - 1.4142135623731*x) + 2.48528137423857*cos(-1.0*t + 1.4142135623731*x + 0.25*pi))*exp(-1.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 1.75735931288071*sin(1.0*t + 1.0*x) - 1.75735931288071*cos(1.0*t + 1.0*x)
  else
    B(3) = (7.75735931288072*sin(1.0*t - 1.4142135623731*x) - 1.75735931288071*cos(1.0*t - 1.4142135623731*x))*exp(-1.0*x)
  endif
