...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 4.8233031891709*sin(1.0*t + 1.0*x) + 0.964660637834179*cos(1.0*t + 1.0*x)
  else
    E(2) = (1.1766968108291*sin(1.0*t - 5.09901951359278*x) + 0.964660637834179*cos(1.0*t - 5.09901951359278*x))*exp(-5.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 4.8233031891709*sin(1.0*t + 1.0*x) - 0.964660637834179*cos(1.0*t + 1.0*x)
  else
    B(3) = (10.8233031891709*sin(1.0*t - 5.09901951359278*x) - 0.964660637834179*cos(1.0*t - 5.09901951359278*x))*exp(-5.0*x)
  endif
