...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 4.10263340389897*sin(1.0*t + 1.0*x) + 1.36754446796632*cos(1.0*t + 1.0*x)
  else
    E(2) = (1.89736659610103*sin(1.0*t - 3.16227766016838*x) + 1.36754446796632*cos(1.0*t - 3.16227766016838*x))*exp(-3.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 4.10263340389897*sin(1.0*t + 1.0*x) - 1.36754446796632*cos(1.0*t + 1.0*x)
  else
    B(3) = (10.102633403899*sin(1.0*t - 3.16227766016838*x) - 1.36754446796632*cos(1.0*t - 3.16227766016838*x))*exp(-3.0*x)
  endif
