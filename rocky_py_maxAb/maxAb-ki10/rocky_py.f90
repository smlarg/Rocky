...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 5.40297768587401*sin(1.0*t + 1.0*x) + 0.540297768587401*cos(1.0*t + 1.0*x)
  else
    E(2) = (0.597022314125993*sin(1.0*t - 10.0498756211209*x) + 0.540297768587401*cos(1.0*t - 10.0498756211209*x))*exp(-10.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 5.40297768587401*sin(1.0*t + 1.0*x) - 0.540297768587401*cos(1.0*t + 1.0*x)
  else
    B(3) = (11.402977685874*sin(1.0*t - 10.0498756211209*x) - 0.540297768587401*cos(1.0*t - 10.0498756211209*x))*exp(-10.0*x)
  endif
