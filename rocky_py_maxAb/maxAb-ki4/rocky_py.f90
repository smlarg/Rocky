...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 4.544786249782*sin(1.0*t + 1.0*x) + 1.1361965624455*cos(1.0*t + 1.0*x)
  else
    E(2) = (1.455213750218*sin(1.0*t - 4.12310562561766*x) + 1.1361965624455*cos(1.0*t - 4.12310562561766*x))*exp(-4.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 4.544786249782*sin(1.0*t + 1.0*x) - 1.1361965624455*cos(1.0*t + 1.0*x)
  else
    B(3) = (10.544786249782*sin(1.0*t - 4.12310562561766*x) - 1.1361965624455*cos(1.0*t - 4.12310562561766*x))*exp(-4.0*x)
  endif
