...
subroutine twod_ps( ps_n1, ps_n2, x1min, x1max, x2min, x2max, np, data1, data2, w, q, ps )
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  if (x <= 0) then
    E(2) = 6.0*sin(1.0*t - 1.0*x) - 3.31671842700025*sin(1.0*t + 1.0*x) + 1.65835921350013*cos(1.0*t + 1.0*x)
  else
    E(2) = (2.68328157299975*sin(1.0*t - 2.23606797749979*x) + 1.65835921350013*cos(1.0*t - 2.23606797749979*x))*exp(-2.0*x)
  endif
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  if (x <= 0) then
    B(3) = 6.0*sin(1.0*t - 1.0*x) + 3.31671842700025*sin(1.0*t + 1.0*x) - 1.65835921350013*cos(1.0*t + 1.0*x)
  else
    B(3) = (9.31671842700025*sin(1.0*t - 2.23606797749979*x) - 1.65835921350013*cos(1.0*t - 2.23606797749979*x))*exp(-2.0*x)
  endif
