...
PROGRAM boris_push
 ...
 SUBROUTINE b_field(x,t,B)
  ...
  B = 0
  IF (x(1) <= 0) THEN
   B(3) =  B0 * ( -cos(x(1) - t) - cos(x(1) + t - 0.200335) )
  ELSE
   B(3) = 1.989975 * B0 * exp( -9.94987 * x(1) ) * sin(t + 1.47027)
  END IF
 ...
 SUBROUTINE e_field(x,t,E)
  ...
  E = 0
  IF (x(1) <= 0) THEN
   E(2) =  -E0 * ( cos(x(1) - t) - cos(x(1) + t - 0.200335) )
  ELSE
   E(2) =  -0.2 * E0 * exp( -9.94987 * x(1) ) * cos(t + 1.47027)
  END IF
