...
!   dist_func = 1.0
 ...
 SUBROUTINE e_field(Y,t,E)
  ...
  x = Y(1)
  IF (x <= 0) THEN
   E(2) =  E0 * ( 10./29.*cos(t+x) + sin(t-x) - 25./29.*sin(t+x))
  ELSE
   E(2) =  E0 * ( 10./29.*exp(-5*x)*cos(t-x) + 4./29.*exp(-5*x)*sin(t-x) )
  END IF
 ...
 SUBROUTINE b_field(Y,t,B)
  ...
  x = Y(1)
  IF (x <= 0) THEN
   B(3) =  B0 * ( -10./29.*cos(t+x) + sin(t-x) + 25./29.*sin(t+x) )
  ELSE
   B(3) =  B0 * ( -10./29.*exp(-5*x)*cos(t-x) + 54./29.*exp(-5*x)*sin(t-x) )
  END IF
