...
PROGRAM boris_push
 ...
 SUBROUTINE print_out_p1x1(particle_array) !this title is shitty
  ...
  do i = 1, UBOUND(particle_array,2)
   ...
   IF (p1_int > 0 .and. p1_int < 201 .and. x1_int > 0 .and. x1_int < 201) &
    & p1x1(x1_int,p1_int) = p1x1(x1_int,p1_int) + particle_array(7,i)
  END do
  !
  call write_p1x1(p1x1)
 END SUBROUTINE print_out_p1x1

 SUBROUTINE print_out_p2p1(particle_array) !this title is shitty
  REAL, DIMENSION(:,:), INTENT(IN) :: particle_array
  ...
  END do
  !
  call write_p2p1(p2p1)
 END SUBROUTINE print_out_p2p1

 SUBROUTINE write_p1x1(p1x1)
  REAL,  DIMENSION(:,:), INTENT(IN) :: p1x1
  INTEGER :: i,j
  OPEN(UNIT=9,FILE='p1x1.dat',STATUS='replace',FORM='unformatted')
  WRITE(UNIT=9) p1x1
  CLOSE(UNIT=9)
 END SUBROUTINE write_p1x1
