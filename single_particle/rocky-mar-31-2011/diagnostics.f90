...
contains
 ...
 SUBROUTINE write_dist_energy(particle_array)
  REAL, DIMENSION(:,:), INTENT(IN) :: particle_array
  REAL, DIMENSION(200) :: dist, dist_fuzz
  INTEGER :: e_int, i
  real :: smallest_none_zero = 1
  real, parameter :: e_range = 30.
  !
  dist = 0
  dist_fuzz = 0
  do i = 1, UBOUND(particle_array,2)
   e_int = energy(particle_array(4:6,i)) &
   & / ( e_range / (ubound(dist,1)-1) )
   IF (e_int > 0 .and. e_int .le. ubound(dist,1) )  &
    & dist(e_int) = dist(e_int) + particle_array(7,i)
  END do
  do i = 2, ubound(dist_fuzz,1) - 1
   dist_fuzz(i) = (dist(i-1) + dist(i) + dist(i+1))/3.0
  end do
 ...
 END SUBROUTINE write_dist_energy
 !
 ...
 SUBROUTINE print_out_p1x1(particle_array)
  REAL, DIMENSION(:,:), INTENT(IN) :: particle_array
  REAL, DIMENSION(126,200) :: p1x1
  INTEGER :: p1_int, x1_int, i
  !
  p1x1 = 0
  do i = 1, UBOUND(particle_array,2)
   p1_int = (particle_array(4,i) + 5  + 0.001) * 10
   x1_int = (particle_array(1,i) + 3.15  + 0.001) * 10
   IF (p1_int > 0 .and. p1_int < 201  &
    & .and. x1_int > 0 .and. x1_int < 127) &
 ...
 SUBROUTINE print_out_p1p2(particle_array)
  REAL, DIMENSION(:,:), INTENT(IN) :: particle_array
  REAL, DIMENSION(200,300) :: p1p2
  INTEGER :: p1_int, p2_int, i
  !
  p1p2 = 0
  do i = 1, UBOUND(particle_array,2)
   p1_int = (particle_array(4,i) + 5  + 0.001) * 10
   p2_int = (particle_array(5,i) + 15  + 0.001) * 10
   IF (p1_int > 0 .and. p1_int < 201  &
    & .and. p2_int > 0 .and. p2_int < 301) &
    & p1p2(p1_int,p2_int) = p1p2(p1_int,p2_int) + particle_array(7,i)
  END do
  !
  OPEN(UNIT=9,FILE='p1p2.dat',STATUS='replace',FORM='unformatted')
  WRITE(UNIT=9) p1p2
  CLOSE(UNIT=9)
 END SUBROUTINE print_out_p1p2
